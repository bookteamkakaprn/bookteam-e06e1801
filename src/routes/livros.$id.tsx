...
function LivroDetalhesPage() {
  ...
  const { data: eventos = [], isLoading: carregandoEventos } = useQuery({
    ...
    queryFn: async () => { const { data, error } = await supabase.from("eventos").select("id,titulo,data,hora,cidade,local,valor,livro_id").eq("livro_id", id).gte("data", new Date().toISOString().slice(0, 10)).order("data", { ascending: true }); if (error) throw error; return (data ?? []) as Evento[]; },
  });
  const { data: turmas = [] } = useQuery({
    queryKey: ["turmas-livro-publico", id], enabled: !isPlaceholder,
    queryFn: async () => { const { data, error } = await supabase.from("turmas").select("id,nome,turma,data_inicio,data_fim,horario,sala,professor,vagas_max,vagas_restantes,status").eq("livro_id", id).gte("data_inicio", new Date().toISOString().slice(0, 10)).order("data_inicio", { ascending: true }); if (error) throw error; return (data ?? []) as Turma[]; },
  });
  ...
  return <div className="min-h-screen min-w-0 overflow-x-hidden bg-background text-foreground">
    ...
    <main className="mx-auto max-w-6xl min-w-0 px-3 py-7 sm:px-4 sm:py-10 md:px-8 md:py-14"><div className="grid min-w-0 gap-7 md:grid-cols-[260px_minmax(0,1fr)] lg:grid-cols-[300px_minmax(0,1fr)]">
      ...
      <div className="min-w-0"><Badge variant="outline" className="border-gold/30 text-gold">{livro?.categoria || "Jornada"}</Badge><h1 className="mt-3 max-w-3xl break-words font-serif text-2xl font-semibold leading-tight sm:text-3xl md:text-4xl">{titulo}</h1>{livro?.autor && <p className="mt-2 flex items-center gap-2 text-xs text-muted-foreground sm:text-sm"><User className="h-4 w-4" />{livro.autor}</p>}
        {primeiroEvento && <div className="mt-5 flex flex-col gap-3 rounded-xl border border-gold/20 bg-card/50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Próxima turma</p><p className="mt-1 text-xs text-muted-foreground">Escolha a turma e faça sua inscrição.</p></div><Button asChild className="w-full bg-gold text-primary-foreground hover:bg-gold/90 sm:w-auto"><Link to="/inscricao/$eventoId" params={{ eventoId: primeiroEvento.id }}>Inscreva-se<ArrowRight className="ml-2 h-4 w-4" /></Link></Button></div>}
        <section className="mt-7"><h2 className="font-serif text-xl font-semibold sm:text-2xl">Próximas turmas</h2><p className="mt-1 text-xs text-muted-foreground sm:text-sm">As turmas abertas aparecem aqui automaticamente.</p>{carregandoEventos && <div className="mt-4 h-28 animate-pulse rounded-xl bg-muted" />}{!carregandoEventos && eventos.length === 0 && turmas.length === 0 && <div className="mt-4 rounded-xl border border-border/60 bg-card/40 p-4 text-sm text-muted-foreground">Ainda não há turma aberta para este livro.</div>}<div className="mt-4 space-y-3">{eventos.map((evento) => <div key={evento.id} className="rounded-xl border border-border/60 bg-card/60 p-4"><div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="break-words font-serif text-base font-semibold">{evento.titulo}</p><div className="mt-2 grid gap-1.5 text-xs text-muted-foreground"><span><Calendar className="mr-1 inline h-3.5 w-3.5 text-gold" />{new Date(`${evento.data}T00:00:00`).toLocaleDateString("pt-BR")}</span>{evento.hora && <span><Clock className="mr-1 inline h-3.5 w-3.5 text-gold" />{evento.hora.slice(0,5)}</span>}{(evento.local || evento.cidade) && <span><MapPin className="mr-1 inline h-3.5 w-3.5 text-gold" />{[evento.local,evento.cidade].filter(Boolean).join(", ")}</span>}</div></div><Button asChild className="w-full shrink-0 bg-gold text-primary-foreground sm:w-auto"><Link to="/inscricao/$eventoId" params={{ eventoId: evento.id }}>Inscreva-se<ArrowRight className="ml-2 h-4 w-4" /></Link></Button></div></div>)}{!eventos.length && turmas.filter((t) => t.status === "aberta" && (t.vagas_max <= 0 || t.vagas_restantes > 0)).map((turma) => <div key={turma.id} className="rounded-xl border border-border/60 bg-card/60 p-4"><div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="font-semibold">{turma.turma || turma.nome || "Próxima turma"}</p><p className="mt-1 text-xs text-muted-foreground">{turma.data_inicio ? `Início: ${new Date(`${turma.data_inicio}T00:00:00`).toLocaleDateString("pt-BR")}` : "Data a definir"}{turma.horario ? ` · ${turma.horario}` : ""}</p></div><Button asChild className="w-full bg-gold text-primary-foreground sm:w-auto"><Link to="/cadastro/$turmaId" params={{ turmaId: turma.id }}>Inscreva-se<ArrowRight className="ml-2 h-4 w-4" /></Link></Button></div></div>)}</div></section>
      </div></div></main>
  </div>;
}
