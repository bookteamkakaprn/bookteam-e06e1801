...
function LivrosSection() {
  ...
  // Placeholder posters se banco vazio
  const placeholders = [
  ...
  return (
    <section id="livros" className="relative border-t border-border/40 bg-gradient-to-b from-background to-card/40 py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        <div className="max-w-2xl">
          <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-gold">
            Biblioteca
          ...
          <h2 className="mt-3 font-serif text-3xl font-semibold md:text-4xl lg:text-5xl">
            Livros que nos formam
          </h2>
          <p className="mt-4 text-[15px] text-foreground/70">
            Leituras cristãs que sustentam nossos encontros e formam a cultura
            de amor e honra.
          </p>
        </div>
        ...
        <div className="mt-12 grid grid-cols-2 gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {(livros.length > 0
            ? livros.map((l) => ({ id: l.id, titulo: l.titulo, autor: l.autor, imagem_url: l.imagem_url }))
            : placeholders.map((titulo, i) => ({ id: String(i), titulo, autor: "", imagem_url: null }))
          ).map((l) => (
            <article key={l.id} className="group">
              <div className="poster-hover shadow-book relative aspect-[2/3] overflow-hidden rounded-xl bg-gradient-wine">
                {l.imagem_url ? (
                  <img
                    src={l.imagem_url}
                    alt={l.titulo}
                  ...
                  <div className="flex h-full w-full flex-col items-center justify-center p-4 text-center">
                    ...
                    <p className="mt-3 font-serif text-sm font-semibold italic text-foreground/90">
                      {l.titulo}
                    </p>
                  </div>
                )}
                ...
                <div className="pointer-events-none absolute inset-x-0 bottom-0 translate-y-2 p-3 opacity-0 transition-all group-hover:translate-y-0 group-hover:opacity-100">
                  <p className="line-clamp-2 text-xs font-semibold text-foreground">{l.titulo}</p>
                  {l.autor && <p className="text-[10px] text-foreground/70">{l.autor}</p>}
                </div>
              </div>
              <p className="mt-3 line-clamp-2 font-serif text-[13px] font-semibold text-foreground/90">
                {l.titulo}
              </p>
              {l.autor && (
                <p className="text-[11px] text-foreground/60">{l.autor}</p>
              )}
            </article>
          ))}
        </div>
