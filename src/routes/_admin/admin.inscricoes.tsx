...
function AdminInscricoes() {
  ...
  const liberar = useMutation({
    mutationFn: async (inscricao: Inscricao) => {
      ...
      await supabase.from("notificacoes").insert({ participante_id: participanteId, titulo: "Você pode iniciar sua jornada!", mensagem: `Seu pagamento foi aprovado e sua inscrição em ${livro} está confirmada. Você já pode iniciar sua jornada.` });
    ...
    onSuccess: () => { toast.success("Aluno liberado para iniciar. A notificação foi registrada e o e-mail será enviado quando o serviço de e-mail estiver configurado."); qc.invalidateQueries({ queryKey: ["admin-inscricoes"] }); qc.invalidateQueries({ queryKey: ["admin-alunos"] }); },
