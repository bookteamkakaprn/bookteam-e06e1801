...
import { Users, Calendar, CreditCard, DollarSign, CheckCircle2, Clock, BookOpen, GraduationCap } from "lucide-react";

export const Route = createFileRoute("/_admin/admin")({
  head: () => ({ head: undefined, meta: [{ title: "Área administrativa — Book Clube" }, { name: "robots", content: "noindex" }] }),
...
function AdminDashboard() {
  ...
  return (
    <div className="w-full min-w-0 space-y-6">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider text-primary">Book Team</p>
        <h1 className="break-words font-serif text-2xl font-bold sm:text-3xl">Área administrativa</h1>
        <p className="text-sm text-muted-foreground">Visão geral do Book Team.</p>
      </div>
      <section>
        <h2 className="font-serif text-xl font-semibold">Resumo</h2>
        <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {cards.map(({ title, value, icon: Icon, href }) => (
            <Card key={title} className="min-w-0 transition-colors hover:border-primary/50">
              <Link to={href} className="block">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">{title}</CardTitle>
                  <Icon className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent><p className="break-words font-serif text-2xl font-bold">{value}</p></CardContent>
              </Link>
            </Card>
          ))}
        </div>
