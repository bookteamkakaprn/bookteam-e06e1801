...
function CertificadosPage(){
  const {user}=useAuth();
  const q=useQuery({enabled:!!user,queryKey:["meus-certificados",user?.id],queryFn:async()=>{const s=await getDocs(query(collection(db,"certificados"),where("participante_id","==",user!.id)));return s.docs.map(d=>({id:d.id,...d.data()})) as Cert[];}});
  return <div className="space-y-5"><div><h1 className="font-serif text-3xl font-bold">Certificados</h1><p className="text-muted-foreground">Seus certificados ficam disponíveis aqui após a conclusão das atividades.</p></div>{q.isLoading&&<p className="text-sm text-muted-foreground"><Loader2 className="mr-1 inline h-4 w-4 animate-spin"/>Carregando…</p>}{q.error&&<p className="text-sm text-destructive">Não foi possível carregar seus certificados.</p>}{!q.isLoading&&!q.error&&(q.data??[]).length===0&&<Card><CardContent className="flex items-center gap-3 p-6 text-sm text-muted-foreground"><Award className="h-5 w-5"/>Nenhum certificado disponível ainda.</CardContent></Card>)}{(q.data??[]).map(c=><Card key={c.id}><CardContent className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-serif text-lg font-semibold">{c.evento_titulo??c.livro_titulo??"Certificado"}</p><p className="text-xs text-muted-foreground">{c.data_emissao?`Emitido em ${new Date(c.data_emissao+"T00:00:00").toLocaleDateString("pt-BR")}`:"Certificado emitido"}{c.carga_horaria?` · ${c.carga_horaria}h`:""}</p></div>{c.pdf_url&&<Button asChild variant="outline" size="sm"><a href={c.pdf_url} target="_blank" rel="noreferrer">Abrir PDF<ExternalLink className="ml-2 h-4 w-4"/></a></Button>}</CardContent></Card>)}</div>;
}
