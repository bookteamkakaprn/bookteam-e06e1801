...
type EventoRow = {
  ...
  valor: number;
  vagas: number;
  livros: { titulo: string; trilhas: { nome: string; cor: string | null } | null } | null;
};
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("eventos")
        .select("id, titulo, descricao, data, hora, cidade, local, valor, vagas, livros(titulo, trilhas(nome, cor))")
        .eq("status", "aberto")
        .gte("data", today)
        .order("data");
