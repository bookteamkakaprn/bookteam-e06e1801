...
function InscricaoPage() {
  ...
  const qEvento = useQuery({ queryKey: ["inscricao-evento", eventoId], enabled: !!eventoId, queryFn: async () => { const snap = await getDoc(doc(db, "eventos", eventoId)); return snap.exists() ? ({ id: snap.id, ...snap.data() } as Evento) : null; } });
  const criar = useMutation({ mutationFn: async () => {
    if (!user) throw new Error("Faça login para continuar."); const e=qEvento.data; if (!e) throw new Error("Encontro não encontrado."); if (!aceite) throw new Error("Aceite os termos para continuar.");
    const existente = await getDocs(query(collection(db,"inscricoes"), where("participante_id","==",user.id), where("evento_id","==",e.id)));
    if (!existente.empty) return { id: existente.docs[0].id };
    const ref = await addDoc(collection(db,"inscricoes"), { participante_id:user.id, evento_id:e.id, status:"aguardando_pagamento", created_at:new Date().toISOString(), updated_at:new Date().toISOString() });
    await addDoc(collection(db,"pagamentos"), { inscricao_id:ref.id, participante_id:user.id, evento_id:e.id, evento_titulo:e.titulo, evento_data:e.data, valor:Number(e.valor??0), status:"aguardando", created_at:new Date().toISOString(), updated_at:new Date().toISOString() });
    return { id: ref.id };
  ...
  return <div className="space-y-6"><Button asChild variant="ghost" size="sm"><Link to="/eventos"><ArrowLeft className="mr-2 h-4 w-4"/>Voltar aos encontros</Link></Button>
