...
function MatriculaPage(){
  const {inscricaoId}=Route.useParams(); const {user}=useAuth(); const qc=useQueryClient(); const [file,setFile]=useState<File|null>(null); const [obs,setObs]=useState("");
  const q=useQuery({enabled:!!user,queryKey:["matricula",inscricaoId],queryFn:async()=>{const s=await getDoc(doc(db,"inscricoes",inscricaoId)); if(!s.exists()||s.data().participante_id!==user!.id)return null; const i={id:s.id,...s.data()} as Inscricao; const ev=await getDoc(doc(db,"eventos",i.evento_id)); const ps=await getDocs(query(collection(db,"pagamentos"),where("inscricao_id","==",inscricaoId))); return {inscricao:i,evento:ev.exists()?({id:ev.id,...ev.data()} as Evento):null,pagamento:ps.empty?null:({id:ps.docs[0].id,...ps.docs[0].data()} as Pag)};}});
  const enviar=useMutation({mutationFn:async()=>{if(!user||!q.data)throw new Error("Matrícula não encontrada");if(!file)throw new Error("Selecione o comprovante");if(file.size>MAX_SIZE)throw new Error("Arquivo maior que 10 MB");if(!ALLOWED.includes(file.type))throw new Error("Formato inválido — use PDF, JPG, JPEG ou PNG");const ext=file.name.split(".").pop()?.toLowerCase()??"bin";const path=`comprovantes/${user.id}/${inscricaoId}/${Date.now()}.${ext}`;const storageRef=ref(storage,path);await uploadBytes(storageRef,file,{contentType:file.type});const url=await getDownloadURL(storageRef);if(q.data.pagamento){await updateDoc(doc(db,"pagamentos",q.data.pagamento.id),{comprovante_url:url,observacao:obs||null,status:"aguardando",updated_at:new Date().toISOString()});}else{await addDoc(collection(db,"pagamentos"),{inscricao_id:inscricaoId,participante_id:user.id,evento_id:q.data.inscricao.evento_id,evento_titulo:q.data.evento?.titulo??"Encontro",valor:Number(q.data.evento?.valor??0),comprovante_url:url,status:"aguardando",observacao:obs||null,created_at:new Date().toISOString(),updated_at:new Date().toISOString()});}},onSuccess:()=>{toast.success("Comprovante enviado!");setFile(null);setObs("");qc.invalidateQueries({queryKey:["matricula",inscricaoId]});},onError:(e:Error)=>toast.error(e.message)});
  if(q.isLoading)return <p className="text-muted-foreground">Carregando…</p>; if(!q.data)return <div className="space-y-3"><p className="text-muted-foreground">Matrícula não encontrada.</p><Button asChild variant="link" className="px-0"><Link to="/inicio">Voltar ao painel</Link></Button></div>;
